...
const isValidHtml = (html: TParse): boolean => html instanceof HTMLElement && html.tagName !== null;
...
const getCountByTag = (
    ...
    html.childNodes.forEach((item) =>
        getCountByTag(item as TParse, analysedData)
    );

    return analysedData;
}

const extractMostCommonTag = (tagsCount: IAnalysedData): string => Object.keys(tagsCount).reduce(
    (prev: string, current: string) => (tagsCount[prev] > tagsCount[current]) ? prev : current
);

export interface IExtractedPaths {
    [name: string]: number
}
const extractPaths = (html: TParse, path: string = '', results: IExtractedPaths = {}): IExtractedPaths => {
    if (isValidHtml(html)) {
        const { tagName } = html as HTMLElement;
    ...
    html.childNodes.forEach((item) =>
        extractPaths(item as TParse, path, results)
    );
...
const extractMostCommonTagLongestPath = (pathesCounts: IExtractedPaths, mostCommonTag: string): IMostCommonTagLongestPath => {
...
export default { getCountByTag, extractMostCommonTag, extractPaths, extractMostCommonTagLongestPath };